...
@media (max-width: 1023px) {
  ...
  /* The Files action stays in-flow inside headerActions. On mobile headerActions
     becomes a four-column grid:
       [mode] [subagent] [jobs] [files]
     The two outer 1fr columns balance the row, which keeps subagent (and, when
     present, jobs) near the horizontal center while Files is pinned to the far
     right. Unlike an absolutely-pinned button, it can never be covered or
     pushed out by the subagent/jobs triggers. */
  [data-mobile-nav="files"] {
  ...
  /* Mode label (AgentPresetLabel): the official pill uses inline-flex, whose
     ...
     block container, keep its icon absolutely positioned on the left, and cap
     its width so the subagent and Files clusters keep room on a phone. */
  ...
  /* Subagent/jobs helpers: keep their triggers on one line even when the grid
     column is tight (the official trigger text wraps to one CJK char per line
     otherwise). */
  ...
  /* Subagent catalog sits in column 2; jobs list sits in column 3; Files is
     pinned to the final column regardless of DOM order. */
  [data-phase] header [class$="_root"]:has(> button[aria-haspopup="tree"]) {
    grid-column: 2 !important;
    grid-row: 1 !important;
    justify-self: center !important;
  }
  [data-phase] header [class$="_root"]:has(> button[class$="_trigger"]):not(:has(> button[aria-haspopup="tree"])) {
