    ...
    return {
        ...
        dispose: () => {
            ...
            if (tpsOrigin !== null && tpsOrigin.parent.isConnected) {
                const tps = [...tpsOrigin.parent.ownerDocument.querySelectorAll('div')].find((el) => el.children.length === 0 && /^TPS\s+\d/.test((el.textContent ?? '').trim()));
                if (tps !== undefined)
...
function useMobile() {
    const [mobile, setMobile] = (0, react_1.useState)(() => window.matchMedia(exports.MOBILE_QUERY).matches);
    (0, react_1.useEffect)(() => {
        const query = window.matchMedia(exports.MOBILE_QUERY);
