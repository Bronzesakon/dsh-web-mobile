...
function installOverlayInteractions(ctx) {
    installMobileEffect(ctx, 'dsh-mobile-nav: drawer close (Escape + navigate)', () => {
        ...
        // document and the drawer would stay open. Handle it on pointerup
        // instead (mouse keeps the click path; pointerType guards keep them
        // exclusive).
        //
        // Self-healing strategy — do NOT depend on the browser synthesizing
        // the tap's click at all. On iOS Safari (and some Android browsers)
        // the synthesized click is routinely suppressed: a close that starts
        // at pointerup moves the row under the finger before click dispatch,
        // and a tap with a few px of drift is classified as a pan. In both
        // cases the row's own onClick never fires: the drawer closes but the
        // conversation does not open. So, one macrotask after pointerup:
        // - if a real click DID arrive, it already ran the row's onClick and
        //   the document-level capture handler above already closed the
        //   drawer — drawerOpen() is false and we do nothing;
        // - if it did NOT arrive, re-dispatch a bubbling click on the row
        //   ourselves. React's delegated listener runs the row's onClick
        //   (opening the session), and the same dispatched click bubbles
        ...
        const onDrawerPointerUp = (event) => {
            ...
            closeTimer = window.setTimeout(() => {
                ...
                const row = target instanceof Element
                    ? target.closest('button[data-dsh-taskboard-entry], button[data-dsh-ssh-entry], [class*="newSession"], [class*="sessionRow"], [class*="searchResultRow"], [class*="searchResultWorkspace"]')
                    : null;
                if (row !== null) {
