...
function apply(ctx) {
    ...
    ctx.effect(() => {
        ...
        if (!narrow.matches)
            return () => { };
        const ensureMoreHint = (root) => {
            let hint = root.querySelector('[data-mobile-nav="stats-more"]');
            if (hint === null) {
        ...
        const observer = new MutationObserver(() => {
            const stats = document.querySelector('[data-mobile-nav="stats"]');
            if (stats !== null)
                ensureMoreHint(stats);
            else
                mark();
        });
