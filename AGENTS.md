...
- 全屏预览（issue #8，2026-08-16 实现）：frame 标记 `data-mobile-preview-full` 由 `[data-mobile-nav="preview-full-toggle"]` 按钮（右上角 right:46px / top:6px 与 suite 折叠按钮同行，z:57）切换；全屏几何在 `[data-mobile-nav="frame"][data-aionui-preview-open][data-mobile-preview-full] [data-aionui-preview-col]`（inset 0 + safe-area padding-top）。预览关闭（index.tsx 的 `closePreview()`：onCollapse 和 visibility watcher 两处）必须同时清 `data-aionui-preview-open` 和 `data-mobile-preview-full`，否则下次预览仍是全屏。`[data-aionui-preview-col] [class$="_tabScroll"]` 要保留 `padding-right: 34px`，否则标签多时最后一个 tab / "+" 按钮会滑到全屏按钮底下。抽屉打开时按钮随 sheet 一起隐藏（该规则必须排在显示规则之后，同 specificity 后者赢）。
- **stacking context 陷阱（2026-08-16 修）**：`shell.overlay` slot 渲染进 `pI_x6G_overlayLayer`（`position:absolute; z-index:20; pointer-events:none`）——它构成 stacking context，层内元素的 z-index 被封印在 20，任何 z>20 的浮层（aionui sheet z:55/56）都会盖住层内按钮。**凡是要浮在 sheet 之上的控件都不能渲染进 overlay 层**：全屏切换按钮用命令式 `document.body.appendChild`（MobileNavOverlay 的 effect，不依赖 react-dom），CSS 用 `body:has([data-mobile-nav="frame"][...])` 选择器读取 frame 标记（按钮已不是 frame 后代）。FAB/backdrop 在层内没问题（无元素与它们重叠）。复现这类问题要在复现页里加同款 z-20 层 + `elementFromPoint` 命中测试，光看 getBoundingClientRect/z-index 不够。
