...
- **`dsh-client-ui-subagent` 0.1.0-rc.6（哈希 `h8S2Va_`）起芯片「点开一闪即退（闪退）」是双开关竞态**：rc.6 删除了 hover 定时器并给 trigger 加了原生 `onClick: () => changeOpen(!open)`（菜单也从 body portal 改回 root 内 `position:absolute` 子节点）。而 `subagent-chip-touch.ts` 的 pointerup（document 捕获，先于 click）仍按旧版逻辑派发合成 ArrowDown/Escape 走键盘路径开/关卡片——于是手机端一次 tap 触发**两次 toggle**：pointerup 的合成 keydown 先把菜单打开（`if (!open) changeOpen(true)`），紧随其后的原生 click 又命中 `onClick` 的 `changeOpen(!open)` 把刚打开的菜单关掉。净效果=菜单闪一帧就消失、芯片看似没反应（用户报「闪退」）。修复：pointerup 派发 keydown 后记录 `toggledTrigger`（1s 宽限窗），document 捕获阶段 click 监听器对同一 trigger 的同一次 tap 的 click 调 `stopPropagation()`——挡住容器级 React 委托（trigger 的 onClick 不再执行）；旧 hover-only 版本没有 onClick，吞 click 无副作用，两代上游行为一致（每次 tap 恰好一次 toggle）。身份比对确保菜单行/其它位置的 click 不受影响。`HOVER_SUBTREE_SELECTOR` 已补 `h8S2Va_root/_menu`（rc.6 无 hover 定时器，该防护在 rc.6 是 no-op，保留以兼容回滚）。坑点：trigger 选择器无哈希（`[aria-haspopup="tree"][aria-expanded]`），所以 rc.6 下旧修复代码仍照常生效——「selector 还活着但上游语义已变」正是竞态来源；上游大改后须重读 served bundle 核对行为，不能只对哈希。
