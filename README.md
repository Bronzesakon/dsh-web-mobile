...
- 安装后宿主无法加载本插件、`dsh web` 启动即崩（#31）：host 半区 ESM 相对导入缺 `.js` 扩展名，Node 下报 `ERR_MODULE_NOT_FOUND`
- 手机上点抽屉历史会话仍可能「抽屉收起但对话不打开」（#32）：主线此前曾改为自愈式补发 click（未随任何版本发布），但在 Opera iOS 等 WebKit 壳上仍会搁浅（兼容 click 被整体抑制）。抽屉收起现改为「导航事实驱动」——观察会话选中状态，确认 React 已完成跳转才收起抽屉，彻底不依赖浏览器合成 click 的时序；已选中行、新会话等目标行为不变
- 手机端点插件市场搜索框触发 iOS 视口强制放大且卡死无法恢复（PR #35，by @BuvkB）：市场搜索框字号提到 16px，iOS 不再对聚焦输入框自动放大；双指缩放不受影响，桌面端无变化
- 刘海屏手机上整个界面能被上滑「抬起」、输入框下方露出空白条、最新一条消息被输入框压住不滚到最新：安全区 `padding-top` 默认叠加在满视口高度之外，把外层文档撑高了一个刘海高度；给 frame 补 `box-sizing: border-box` 让安全区从 100% 高度内扣除，界面恢复恰好一屏、粘底输入框归位
