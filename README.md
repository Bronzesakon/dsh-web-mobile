...
- **点按振动**:点按交互控件(按钮 / 选项卡 / 会话行 / 链接等)触发轻振(Android,`navigator.vibrate`),60ms 节流防连震;设置 → 通用 内提供「点按振动（Mobile）」开关与**振动强度**选择(轻 8ms / 中 15ms / 重 30ms,官方胶囊选择器,开关关闭时置灰;默认开、localStorage 持久化、跨标签页同步,行内署名插件并注明仅支持手机等支持振动的设备);桌面端 no-op;
